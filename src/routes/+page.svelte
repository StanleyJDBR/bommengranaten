...
<script lang="ts">
  import { game, sortedPlayers, currentPlayer, PLAYER_EMOJIS } from '$lib/stores/game';
...
{#if $game.phase === 'setup'}
  <div class="pt-8 pb-12">
    ...
    <div class="pirate-card mb-3">
      <h3 class="font-pirata text-gold-light text-lg mb-4 tracking-wide">⚓ Spelers</h3>
    ...
    <button
      class="btn-gold w-full py-4 text-xl"
      on:click={startGame}
    >⚓ Begin het spel!</button>

    <div class="text-center mt-4">
      <a href="/stats" class="text-foam/50 text-sm hover:text-foam transition-colors">📊 Statistieken bekijken</a>
    </div>
  </div>
...
{:else if $game.phase === 'playing'}
  <div class="pt-4 pb-20">
    <header class="text-center mb-3">
      <h1 class="font-pirata text-gold-light text-2xl" style="text-shadow: 0 0 30px rgba(201,146,42,0.4)">
        ☠️ 1000 B&G
      </h1>
    </header>
    ...
    <!-- Eindronde banner -->
    {#if $game.finalRound}
      <div class="border border-blood-light/45 bg-blood/20 rounded-xl px-4 py-3 mb-3 text-center" style="animation: warnPulse 2s ease-in-out infinite">
        <h3 class="font-pirata text-red-400 tracking-widest">☠️ EINDRONDE ☠️</h3>
        <p class="text-red-400/70 text-sm mt-1">
          {$game.players.find(p => p.id === $game.finalRoundBy)?.name} haalt 6000+!
          Alle anderen krijgen nog één beurt.
        </p>
      </div>
    {/if}
    ...
    <div class="text-center mb-2">
      <button class="btn-ghost px-4 py-2 text-sm" on:click={() => showHistory = !showHistory}>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-ocean/96 border-t border-gold/12 px-4 py-2 flex gap-2 backdrop-blur-md z-50">
      <button class="btn-ghost flex-1 py-2 text-[0.75rem]" on:click={() => game.undo()}>↩ Ongedaan</button>
      <button class="btn-ghost flex-1 py-2 text-[0.75rem]" on:click={() => { if (confirm('Nieuw spel starten?')) game.reset(); }}>🔄 Nieuw spel</button>
      <a href="/stats" class="btn-ghost flex-1 py-2 text-[0.75rem] text-center">📊 Stats</a>
    </div>
  </div>
...
{:else if $game.phase === 'end'}
  <div class="pt-8 pb-12 text-center">
    <header class="mb-6">
      <h1 class="font-pirata text-gold-light text-4xl" style="text-shadow: 0 0 40px rgba(201,146,42,0.5)">🏆 Einde Spel</h1>
      <p class="font-mono text-[0.6rem] uppercase tracking-[4px] text-foam/50 mt-2">De winnaars zijn bekend</p>
      <div class="flex items-center gap-3 mt-4">
        <div class="flex-1 h-px bg-gradient-to-r from-transparent via-gold/35 to-transparent" />
        <span class="text-gold">☠</span>
        <div class="flex-1 h-px bg-gradient-to-r from-transparent via-gold/35 to-transparent" />
      ...
      {#each [1, 0, 2] as podiumIdx, i}
        {#if $sortedPlayers[podiumIdx]}
          ...
          ">
            ...
            <div class="font-pirata text-gold-light text-2xl mt-1">{p.score}</div>
    ...
    <div class="pirate-card text-left mb-6">
      {#each $sortedPlayers as p, i}
        <div class="flex items-center gap-3 py-2.5 border-b border-white/5 last:border-0">
          ...
          <span class="font-pirata text-gold-light text-2xl">{p.score}</span>
        </div>
      {/each}
    </div>

    <button class="btn-gold w-full py-4 text-xl mb-3" on:click={() => game.reset()}>
      ⚓ Nieuw Spel
    </button>
    <a href="/stats" class="block text-foam/50 text-sm hover:text-foam transition-colors">
